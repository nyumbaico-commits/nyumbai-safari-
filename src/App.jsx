...
const [enquiry, setEnquiry] = useState(null); // holds package when modal open
const [form, setForm] = useState({ name: "", email: "", phone: "", dates: "", people: "1",
...
body: JSON.stringify({ ...form, package: enquiry?.title, duration: enquiry?.duration,
});
setFormStatus("sent");
...
border: "1px solid #DDD5C0", background: "#FAF8F3",
fontFamily: "Arial", fontSize: "13px", color: "#1A1510",
...
<button
onClick={(e) => { e.stopPropagation(); setEnquiry(pkg); setFormStatus("id
style={{
marginTop: "14px", width: "100%", padding: "12px",
background: pkg.color, color: "#FFFFFF", border: "none",
...
<div style={{
background: "#FAF8F3", borderRadius: "12px", padding: "28px 24px",
...
<div>
<p style={{ fontSize: "10px", letterSpacing: "0.2em", textTransform: "upperca
<h3 style={{ fontSize: "18px", fontWeight: "normal", color: "#1A1510", <p style={{ fontSize: "11px", color: "#9A8A6A", fontFamily: "Arial", marginTo
</div>
<button onClick={() => setEnquiry(null)} style={{ background: "none", border: "
</div>
fontFa
{formStatus === "sent" ? (
<div style={{ textAlign: "center", padding: "30px 0" }}>
<div style={{ fontSize: "40px", marginBottom: "16px" }}> </div>
<p style={{ fontSize: "16px", color: "#1A1510", fontFamily: "'Georgia', serif
<p style={{ fontSize: "13px", color: "#9A8A6A", fontFamily: "Arial", lineHeig
<button onClick={() => setEnquiry(null)} style={{ marginTop: "20px", padding:
</div>
) : (
<>
<input style={inputStyle} placeholder="Your name *" value={form.name} onChang
<input style={inputStyle} placeholder="Email address *" type="email" value={f
<input style={inputStyle} placeholder="Phone / WhatsApp" value={form.phone} o
<input style={inputStyle} placeholder="Preferred travel dates" value={form.da
<input style={inputStyle} placeholder="Number of travellers" type="number" mi
<textarea style={{ ...inputStyle, height: "90px", resize: "vertical", marginB
{formStatus === "error" && (
<p style={{ fontSize: "12px", color: "#c0392b", fontFamily: "Arial", margin
)}
<button
onClick={handleSubmit}
disabled={formStatus === "sending" || !form.name || !form.email}
style={{
width: "100%", padding: "13px", background: (!form.name || !form.email) ?
color: "#fff", border: "none", borderRadius: "6px",
fontFamily: "'Georgia', serif", fontSize: "14px",
letterSpacing: "0.06em", cursor: (!form.name || !form.email) ? "not-allow
}}
>
{formStatus === "sending" ? "Sending..." : "Send Enquiry"}
</button>
<p style={{ fontSize: "10px", color: "#C0B090", fontFamily: "Arial", textAlig
